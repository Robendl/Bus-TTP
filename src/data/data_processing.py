...
def prepare_data(cfg: Config, device):
    ...
    X_train, X_val, X_test, y_train, y_val, y_test = split_data(df, cfg.training.val_size, cfg.training.test_size,
...
def create_seq_dataloader(cfg: Config, df_time: pd.DataFrame, df_route: pd.DataFrame, device) -> DataLoader:
    dataset = SequenceDataset(df_time, df_route)
    dataLoader = DataLoader(dataset, batch_size=cfg.training.batch_size, shuffle=True)
    return dataLoader
