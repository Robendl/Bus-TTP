...
class LSTMFeedforwardCombination(nn.Module):
    """Two-stream model: an LSTM over road-segment sequences combined with
    a feed-forward stack over trip-level features."""

    def __init__(self, cfg: Config, lstm_input_dim: int, ff_input_dim: int):
        super().__init__()
        ...
        self.lstm = nn.LSTM(
            ...
            num_layers=cfg.model.lstm.num_lstm_layers,
            batch_first=True,
            dropout=dropout,
        )
        lstm_output_dim = (2 if self.bidirectional else 1) * lstm_hidden_dim
    ...
    def forward(self, time_features, padded_routes, lengths):
        if torch.onnx.is_in_onnx_export():
            ...
            packed = pack_padded_sequence(
                padded_routes, lengths, batch_first=True, enforce_sorted=False
            )
        ...
        if self.bidirectional:
            ...
            hn = hn[-1]
        hn = self.lstm_dropout(self.lstm_norm(hn))

        tf = time_features
        for hidden_layer in self.fc_hidden_list:
            tf = self.dropout(self.act(hidden_layer(tf)))

        return self.head(torch.cat((hn, tf), dim=1))
